...
// 对于 workbench 的理解：
// 对核心编辑器提供一整套组合的功能：包含文本搜索、git管理、调试、导航、菜单等功能。 并且对于 monaco 的引用是资源级别而非 npm 包。
...
// TODO: 对于 web 架构的理解有疑问的地方
// 1. 似乎有没有 node 端: (reh) 没有太多有差异的地方，构建的产物差别，只有 reh 这种形式有部分 node 产物。 那么产生差异的地方是哪里呢？
// 2. 对于有 node 端的交付形式，web 在于 node 端连接时，是做了什么事？ node 端跑的扩展进程，是如何与 web 端进行交互的？
