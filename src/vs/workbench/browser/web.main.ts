...
export class BrowserMain extends Disposable {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; configurationService: IWorkbenchConfigurationService; logService: ILogService }> {
		...
		// Remote Agent.  远程代理. TODO: 纯 webide 应该没有 ws 后端支持？ 而 code-server 需要默认提供一个 agent，也就是需要填充一个 webSocketFactory
		// TODO: 问题 1: 读取远程的文件、文件夹一定需要有一个 ws 连接吗？ 纯 webide 读取本地文件的方式是？？？
