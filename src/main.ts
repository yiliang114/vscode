...
async function startup(codeCachePath: string | undefined, nlsConfig: INLSConfiguration): Promise<void> {
	// TODO: 首先会检查用户语言环境配置，如果没有设置默认使用英语？？？
	...
	// Load Main
	// 以前是使用默认的 require 为一个修改过的 loader，用它来加载 src/vs/code/electron-main/main 模块
...
async function resolveNlsConfiguration(): Promise<INLSConfiguration> {
	...
	// TODO: 貌似已经做了优化，这里就会调用config服务，把本地配置加载进来再调用startup
