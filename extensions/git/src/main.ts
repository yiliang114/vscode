...
async function createModel(context: ExtensionContext, logger: LogOutputChannel, telemetryReporter: TelemetryReporter, disposables: Disposable[]): Promise<Model> {
	...
	disposables.push(
		cc,
		new GitFileSystemProvider(model), // .git 文件系统？
		new GitDecorations(model), // 装饰
		new GitTimelineProvider(model, cc), // 时间线
		new GitEditSessionIdentityProvider(model), // TODO: 编辑记录？?
		new TerminalShellExecutionManager(model, logger) // TODO: Termial 中相关操作的管理区？
	);
...
async function isGitRepository(folder: WorkspaceFolder): Promise<boolean> {
	// TODO: 是不是 git 仓库的判断，这里直接判断不是 file 就 false ？？？ 那 vscode-remote 或者其他自定义 fs scheme 肯定不是 file 就不能有 git 原生功能了？
